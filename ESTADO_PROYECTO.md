...
#### Bug corregido: preguntas de ac-ox/sal-ox/ac-hid sin respuesta correcta entre las opciones (2026-09-14)
- **Reporte real de un docente**: un estudiante vio la fórmula HIO₂ (probablemente transcrita como "HBO2" al reportarlo) en el juego y **ninguna de las opciones de respuesta era correcta**.
- **Causa raíz**: `isCorrect()` ya restringía los grupos `ac-ox`, `sal-ox` y `ac-hid` a nomenclatura Tradicional únicamente (curricular, comentario ya existente: "curriculum requires only traditional nomenclature") — pero `renderMC()` (la función que arma las opciones de una pregunta de opción múltiple) **no** aplicaba esa misma restricción: elegía al azar entre `trad`+`stock`+`sist` para decidir qué mostrar como "la respuesta correcta". Cuando el compuesto tenía un nombre Stock o sistemático guardado (ej. HIO₂: `stock:['ácido dioxoyódico (III)']`, `sist:['dioxoyodato (III) de hidrógeno']`, distintos de `trad:['ácido yodoso']`), `renderMC` podía mostrar ese nombre no-tradicional como opción "correcta" — pero al hacer clic, `isCorrect()` la rechazaba por no ser Tradicional, y como los distractores también se generan excluyendo lo que `isCorrect` ya marca como válido, **ninguna de las 4 opciones podía quedar marcada como correcta nunca**.
- **Alcance del bug**: afectaba a 40 de los 403 compuestos del banco — 9 en `ac-hid` (HF, HCl, HBr, HI, H₂S, H₂Se, HCN, H₂Te, HN₃; su `sist` traía el nombre IUPAC tipo "cloruro de hidrógeno" en vez de repetir el Tradicional), 3 en `ac-ox` de la sección "banco adicional" (HIO₂, H₂SO₂, H₂TeO₃; tenían Stock y sistemática reales y distintas), y 28 en `sal-ox` (sales de metales de valencia variable — Fe, Cu, Ni, Co, Sn, Cr, Mn, Ag — más varias sales con `sist` distinto del Tradicional).
- **Corrección**: en los 40 compuestos de esos 3 grupos, `stock` se vació (`[]`) y `sist` se igualó al valor de `trad`, siguiendo el mismo patrón que ya usaban las demás entradas de `ac-ox`. Esto no borra ningún compuesto (siguen siendo 403) ni cambia los `hint`; solo hace que `renderMC`, los distractores (`buildSmartDistractors`) y el arrastrar-palabras (`buildDrag`) —que antes preferían el nombre Stock cuando existía— usen siempre el nombre Tradicional para estos 3 grupos, consistente con lo que `isCorrect()` ya exigía. Verificado con un script que confirma 0 compuestos restantes con `stock`/`sist` no-tradicional en `ac-ox`/`sal-ox`/`ac-hid`, y con una sesión de juego jugada automáticamente en esos 3 grupos sin ninguna pregunta de opción múltiple sin respuesta correcta.
- **Prefijos meta-/piro-/orto-**: se auditó el banco completo buscando esos prefijos en nombres, y no se encontró ninguno — ya se habían eliminado en una limpieza de una sesión anterior (commit `d3b3930`). Si el docente los sigue viendo en el juego, es señal de que el sitio publicado en GitHub Pages tiene una versión de `banco.js` anterior a esa limpieza y falta redesplegarla.
...
- **Dos modos de juego**:
  ...
  - *Modo Estudiante*: el alumno ingresa su código de 6 dígitos; el sistema valida contra el listado oficial (177 estudiantes de 10° y 11°) y muestra nombre y curso.
- **Validación de código migrada al backend (2026-09-10, protección de datos de menores)**: el listado código→nombre vivía como un objeto `STUDENTS` en texto plano dentro de este archivo, público en GitHub Pages — visible con "ver código fuente" para cualquiera, con el nombre completo de cada estudiante. Se eliminó de aquí; ahora `validateCode()` es asíncrona y consulta `GET ?accion=validarCodigo&codigo=...` en `ionom-analytics.gs`, que lee la hoja "Estudiantes" (Código | Nombre | Curso) del Google Sheet — nunca committeada al repositorio. Ventaja adicional: el docente ahora agrega/corrige/quita estudiantes editando esa hoja directamente, sin tocar ni redesplegar `juego.html`. Contrapartida: la validación del código ahora requiere conexión a internet (antes era instantánea y local) y tiene un pequeño retraso mientras responde el Apps Script; se agregó un estado "Verificando…" en el botón y un mensaje de error si falla la conexión.
- Al terminar, envía automáticamente los resultados a Google Sheets vía **Google Apps Script**.
- **Envío de resultados: de `no-cors` opaco a respuesta verificada + reintento (2026-09-11)**: un error real en producción (`doPost` agotando sus 6 reintentos por congestión transitoria de Google) reveló que `sendResults()`/`_post()` usaban `fetch(..., {mode:'no-cors'})`, lo que hace la respuesta **opaca**: el `.then()` de éxito se dispara siempre que la petición de red se complete, sin importar si el backend realmente guardó la fila o devolvió `{ok:false}`. Un estudiante podía ver "✅ Resultados registrados" con su partida perdida, sin ningún aviso. Corregido: el `Content-Type` pasó de `application/json` a `text/plain;charset=utf-8` (evita el preflight `OPTIONS` que Apps Script no responde, igual que antes) pero ya SIN `mode:'no-cors'`, así el frontend puede leer `{ok, error}` de verdad. `sendResults()` ahora reintenta hasta 3 veces (con espera creciente) si el backend responde `ok:false` o falla la red, y solo tras agotar los reintentos muestra el mensaje honesto "📵 Sin conexión al servidor" con los datos para captura manual — antes ese mensaje solo aparecía ante una falla de red, nunca ante un fallo real del servidor.
